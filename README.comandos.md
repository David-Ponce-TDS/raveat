...
administrador que sembró la migración. Con la sesión iniciada, la misma prueba está en
**Mi cuenta → Diagnóstico**, que además muestra contra qué URL está pegando: es la única forma de
verla en el teléfono, donde no hay consola a mano.
...
Sin `-Headers $auth`, el mismo listado responde **401**. Los rechazos de la API se provocan a
propósito: también son una prueba. `Invoke-RestMethod` tira la excepción en rojo con un 400 y
esconde el cuerpo; se lee así:
...
|---|---|---|
| «No se pudo conectar con la API» | La API no está levantada, o la URL apunta a otro lado | Probar `/health`; con sesión, **Mi cuenta → Diagnóstico** muestra la URL y la respuesta |
